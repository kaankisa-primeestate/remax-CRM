...
export const usersApi = {
  ...
  changeEmail: (payload) =>
    apiClient.patch('/users/change-email', payload).then((r) => r.data),
  createBroker: (payload) =>
    apiClient.post('/users/brokers', payload).then((r) => r.data),
  setMonthlyTarget: (agentId, monthlyTarget) =>
    apiClient.patch(`/users/agents/${agentId}/target`, { monthlyTarget }).then((r) => r.data),
  setMonthlyDues: (agentId, monthlyDuesAmount, duesStartDate) =>
    apiClient.patch(`/users/agents/${agentId}/dues`, { monthlyDuesAmount, duesStartDate }).then((r) => r.data),
